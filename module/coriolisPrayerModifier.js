...
export class CoriolisModifierDialog extends FormApplication {
  constructor(chatMessage, origRollData, origRoll) {
    ...
    this.itemModifiers = {};
    if (
      origRollData.rollType != "attribute" &&
      origRollData.rollType != "armor"
    ) {
  ...
  async _updateObject(event) {
    ...
    for (const modifier in this.itemModifiers) {
      if (this.itemModifiers[modifier].checked) {
        this.origRollData.prayerModifiersBonus += parseInt(
          this.itemModifiers[modifier].value
        );
        this.origRollData.prayerModifiers[modifier] = this.itemModifiers[
          modifier
        ];
