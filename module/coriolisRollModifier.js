...
export class CoriolisModifierDialog extends FormApplication {
  constructor(rollData, chatOptions) {
    ...
    this.itemModifiers = rollData.gunnerToChoose
      ? this.crewMembersControlled[this.gunnerChoosen].system.itemModifiers
          .rangedcombat
  ...
  async _onChangeInput(event) {
    ...
    if (event.currentTarget.name.match(/^si_.*$/)) {
      this.itemModifiers[event.currentTarget.name].checked =
        event.currentTarget.checked;
    }
    if (event.currentTarget.name === "gunnerChoosen") {
      this.gunnerChoosen = event.currentTarget.value;
      this.itemModifiers = this.crewMembersControlled[
        this.gunnerChoosen
      ].system.itemModifiers.rangedcombat;
    }
    this.render();
  }
  ...
  async _updateObject(event) {
    ...
    this.rollData.numberOfIgnoredOnes =
      this.rollData.machineGunner + this.rollData.highCapacity;
    this.rollData.itemModifiers = this.itemModifiers;
    if (this.gunnerToChoose) {
      this.rollData.actorType = this.crewMembersControlled[
        this.gunnerChoosen
      ].type;
      this.rollData.gunnerName = this.crewMembersControlled[
        this.gunnerChoosen
      ].name;
      this.rollData.attribute = this.crewMembersControlled[
        this.gunnerChoosen
      ].system.attributes.agility.value;
      this.rollData.skill = this.crewMembersControlled[
        this.gunnerChoosen
      ].system.skills.rangedcombat.value;
      this.rollData.rollTitle =
        game.i18n.localize("YZECORIOLIS.CrewSpotGunner") +
        " " +
        this.rollData.gunnerName +
        "\n- " +
        this.rollData.ship;
    }
